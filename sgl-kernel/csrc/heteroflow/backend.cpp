...
thread_local int CPUBackend::numa_node = -1;
#endif

thread_local int CPUBackend::thread_local_id = -1;

CPUBackend::CPUBackend(int max_thread_num) {
    ...
    for (int i = 1; i < max_thread_num_; i++) {
        workers_[i] = std::thread(&CPUBackend::worker_thread, this, i);
    }
}
...
int CPUBackend::get_thread_num() { return max_thread_num_; }

void CPUBackend::do_work_stealing_job(int task_num,
...
void CPUBackend::process_tasks(int thread_id) {
...
void CPUBackend::worker_thread(int thread_id) {
