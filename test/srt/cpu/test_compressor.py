...
class BenchmarkCompressor(CustomTestCase):

    def _bench(self, fn, warmup=5, iters=50):
    ...
    def test_bench_decode_overlap(self):
        bs, ratio, head_dim, rope_head_dim = 1, 4, 576, 64
        overlap, rotate = True, False
        norm_eps = 1e-6
        coff = 2
        coff_hd = coff * head_dim
        ...
        kv = torch.randn(bs, coff_hd, dtype=torch.float32)
        score = torch.randn(bs, coff_hd, dtype=torch.float32)
        seq_lens = torch.tensor([1024], dtype=torch.int64)
        ...
        def ref_fn():
            pk, ps = pool_kv.clone(), pool_score.clone()
            return compress_decode_ref(
                pk, ps, kv, score, seq_lens, req_pool_indices,
                ape, norm_weight, freqs_cis, ratio, head_dim, rope_head_dim,
                overlap, rotate, norm_eps)
        ...
        ref_ms = self._bench(ref_fn)
        kern_ms = self._bench(kern_fn)
        print(f"\ncompress_decode (bs={bs}, ratio={ratio}, head_dim={head_dim}, overlap={overlap}):")
        print(f"  Reference: {ref_ms:.3f} ms")
        print(f"  Kernel:    {kern_ms:.3f} ms")
        print(f"  Speedup:   {ref_ms / kern_ms:.2f}x")

    def test_bench_extend_overlap(self):
        bs, ratio, head_dim, rope_head_dim = 1, 4, 576, 64
        overlap, rotate = True, False
        norm_eps = 1e-6
        coff = 2
        ...
        pool_kv = torch.randn(max_reqs, state_len, coff_hd, dtype=torch.float32)
        pool_score = torch.randn(max_reqs, state_len, coff_hd, dtype=torch.float32)
        kv = torch.randn(extend_len, coff_hd, dtype=torch.float32)
        score = torch.randn(extend_len, coff_hd, dtype=torch.float32)
        prefix_lens = torch.tensor([0], dtype=torch.int64)
        extend_lens_t = torch.tensor([extend_len], dtype=torch.int64)
        req_pool_indices = torch.tensor([0], dtype=torch.int64)
        ape = torch.randn(ratio, coff_hd, dtype=torch.float32)
        norm_weight = torch.randn(head_dim, dtype=torch.float32).abs() + 0.1
        freqs_cis = torch.randn(max_seq, rope_head_dim, dtype=torch.float32)
        ...
        def ref_fn():
            pk, ps = pool_kv.clone(), pool_score.clone()
            return compress_extend_ref(
                pk, ps, kv, score, prefix_lens, extend_lens_t,
                req_pool_indices, ape, norm_weight, freqs_cis, ratio, head_dim,
                rope_head_dim, overlap, rotate, norm_eps)
        ...
        ref_ms = self._bench(ref_fn)
        kern_ms = self._bench(kern_fn)
        print(f"\ncompress_extend (bs={bs}, ratio={ratio}, head_dim={head_dim}, extend_len={extend_len}):")
        print(f"  Reference: {ref_ms:.3f} ms")
        print(f"  Kernel:    {kern_ms:.3f} ms")
        print(f"  Speedup:   {ref_ms / kern_ms:.2f}x")
