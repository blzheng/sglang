...
class Conv1dLayer(MultiPlatformOp):
    """Drop-in replacement for nn.Conv1d with AMX-optimized CPU path.

    On CPU with AMX support, uses sgl_kernel.conv1d_cpu (im2col + brgemm).
    The CPU path returns [N, L_out, OC] layout directly (avoiding transpose).
    On CUDA/native, falls back to F.conv1d returning [N, OC, L_out].
    """
    ...
    def __init__(
        ...
        if _is_cpu and _is_amx_available:
            self.quant_method = PackWeightMethod(
                weight_names=["weight"], is_conv1d=True
            )
    ...
    def forward_cpu_nhwc(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Use this when the caller needs [N, L_out, OC] anyway (e.g. Whisper encoder).
